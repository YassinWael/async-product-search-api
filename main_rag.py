...
# Loaded once at startup — small 22MB model, runs fully local, no API key needed
...
app = FastAPI(title="Product Search API (RAG)", lifespan=lifespan)
...
def rank_with_embeddings(query, results):
    """
    Re-scores each product's relevance_score using cosine similarity
    between the query embedding and each product name embedding.

    Uses FAISS for the similarity search and sentence-transformers
    (all-MiniLM-L6-v2) for the embeddings.
    """
    if not results:
        return results
    ...
    # Embed everything — normalize=True gives unit vectors so inner product == cosine similarity
    product_embeddings = embedding_model.encode(product_names, normalize_embeddings=True)
    ...
    scores, indices = index.search(np.array(query_embedding, dtype=np.float32), len(results))

    # Write the cosine similarity back as the relevance score
    for score, idx in zip(scores[0], indices[0]):
        results[idx].relevance_score = round(float(score), 4)

    logger.info("embedding re-ranking done for query=%s, %d products", query, len(results))
    return results
...
def is_listing_like_page(link, title):
    text = f"{link} {title}".lower()
    hints = [
        "/category", "/categories", "/shop/", "/laptops",
        "/collections/", "/c/", "best prices", "shop ", "online",
    ]
    ...
    for store in stores[:max_stores]:
        ...
        results.append(
            ProductResult(
                ...
                relevance_score=0.0,  # will be replaced by embedding score
            )
        )
    return results
...
async def search_products(query, region, max_products=5, max_stores=3):
    ...
    # Re-rank using embeddings instead of word overlap
    flat_results = rank_with_embeddings(query, flat_results)
...
<head>
  ...
  <title>Product Search (RAG)</title>
  <style>
    body {{
      margin: 0;
